...
import { ClipboardList, HelpCircle, ShieldCheck, ClipboardCheck } from "lucide-react";
import { getCurrentUser } from "@/lib/dal";
import { can } from "@/lib/permissions";
import { getEngineerDashboard } from "@/server/engineering";
...
// PRD_Engineer_Dashboard §5 — exact seven-region layout.
export default async function EngineerDashboardPage() {
  const { tenantId, role, user } = await getCurrentUser();
  if (!can(role, "PROJECTS", "READ")) redirect("/dashboard/executive");

  const data = await getEngineerDashboard(tenantId, { userId: user.id, role });
  const { t } = await getT();
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatTile label={t("dashboards.engineer.technicalTasks")} value={String(data.cues.technicalTasks)} icon={ClipboardList} iconColor="#2457C5" iconBg="#E4ECFB" href="/tasks" />
        <StatTile label={t("dashboards.engineer.openRfis")} value={String(data.cues.openRfis)} icon={HelpCircle} iconColor="#B76E00" iconBg="#FBECD2" href="/dashboard/engineering/rfis?status=open" />
        <StatTile label={t("dashboards.engineer.inspections")} value={String(data.cues.inspections)} icon={ShieldCheck} iconColor="#1A7F4E" iconBg="#E2F4EA" href="/dashboard/engineering/inspections" />
        <StatTile label={t("dashboards.engineer.approvals")} value={String(data.cues.approvals)} icon={ClipboardCheck} iconColor="#4a3aa7" iconBg="#EEEAFB" href="/dashboard/engineering/approvals" />
      </div>
          ...
          {data.myProjects.length === 0 ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {data.myProjects.map((p) => (
                <Link key={p.id} href={`/dashboard/engineering/projects/${p.id}`} className="rounded-xl border border-border p-4 hover:border-gold/60 transition-colors">
                  ...
                  <div className="flex flex-wrap gap-1.5 mt-2">
                    ...
                    {p.attentionChips.inspections > 0 && <Chip label={`${p.attentionChips.inspections} inspections`} />}
                    {p.attentionChips.coordination > 0 && <Chip label={`${p.attentionChips.coordination} coordination`} />}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <Card>
          <CardHeader>
            <CardTitle>{t("dashboards.engineer.technicalStatus")}</CardTitle>
          </CardHeader>
          <CardContent className="space-y-3">
            <StatusRow label={t("nav.engineeringPackages")} counts={data.technicalStatus.packages} />
            <StatusRow label={t("nav.specifications")} counts={data.technicalStatus.specifications} />
            <StatusRow label={t("nav.calculations")} counts={data.technicalStatus.calculations} />
            <StatusRow label={t("nav.submittals")} counts={data.technicalStatus.submittals} />
            <StatusRow label={t("nav.inspections")} counts={data.technicalStatus.inspections} />
            <div className="flex items-center justify-between text-sm">
              <span className="text-ink-muted">{t("nav.rfis")}</span>
              <span className="text-ink">
                {data.technicalStatus.rfis.open} {t("dashboards.architect.open")} · {data.technicalStatus.rfis.overdue} {t("dashboards.architect.overdue")}
              </span>
            </div>
            <div className="flex items-center justify-between text-sm">
              <span className="text-ink-muted">{t("nav.coordination")}</span>
              <span className="text-ink">{data.technicalStatus.coordination}</span>
            </div>
          </CardContent>
        </Card>
      </div>
