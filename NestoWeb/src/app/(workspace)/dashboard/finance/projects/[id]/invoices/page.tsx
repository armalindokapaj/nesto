...
// §10 "Invoices" tab — sales AND purchase invoices filtered to project.
export default async function ProjectInvoicesTabPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const rows = await db.invoice.findMany({ where: { tenantId, projectId: id, type: { in: ["INVOICE", "BILL"] } }, orderBy: { issuedDate: "desc" } });
  const { t } = await getT();

  return (
    <div className="space-y-6">
      <ProjectFinanceTabHeader projectId={id} projectName={project.name} projectStatus={project.status} active="invoices" />
      <Card>
        <CardHeader>
          <CardTitle>{t("dashboards.finance.tabInvoices")}</CardTitle>
        </CardHeader>
