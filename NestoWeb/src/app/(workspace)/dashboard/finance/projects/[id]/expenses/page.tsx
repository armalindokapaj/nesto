...
export default async function ProjectExpensesTabPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const rows = await db.invoice.findMany({ where: { tenantId, projectId: id, type: { in: ["EXPENSE", "BILL"] } }, orderBy: { issuedDate: "desc" } });
  const { t } = await getT();

  return (
    <div className="space-y-6">
      <ProjectFinanceTabHeader projectId={id} projectName={project.name} projectStatus={project.status} active="expenses" />
      <Card>
        <CardHeader>
          <CardTitle>{t("dashboards.finance.tabExpenses")}</CardTitle>
        </CardHeader>
