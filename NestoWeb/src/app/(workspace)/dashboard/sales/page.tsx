  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatTile label={t("dashboards.sales.pipelineValue")} value={formatCurrency(data.kpis.pipelineValue)} icon={TrendingUp} iconColor="#1A7F4E" iconBg="#E2F4EA" href="/clients/pipeline" />
        ...
        <StatTile
          ...
          value={data.kpis.revenue === null ? t("dashboards.sales.restricted") : formatCurrency(data.kpis.revenue)}
          icon={Wallet}
          iconColor="#1A7F4E"
          iconBg="#E2F4EA"
          href={data.kpis.revenue === null ? undefined : "/reports"}
        />
        <StatTile
          label={t("dashboards.sales.outstandingPayments")}
          value={data.kpis.outstandingPayments === null ? t("dashboards.sales.restricted") : formatCurrency(data.kpis.outstandingPayments)}
          icon={Receipt}
          iconColor="#B76E00"
          iconBg="#FBECD2"
          href={data.kpis.outstandingPayments === null ? undefined : "/clients/payments"}
        />
          ...
          {!data.financial_summary ? (
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              <div>
                ...
                <p className="text-lg font-semibold text-ink">{formatCurrency(data.financial_summary.contractValue)}</p>
              </div>
              <div>
                <p className="text-xs text-ink-muted">{t("dashboards.sales.amountPaid")}</p>
                <p className="text-lg font-semibold text-ink">{formatCurrency(data.financial_summary.amountPaid)}</p>
              </div>
              <div>
                <p className="text-xs text-ink-muted">{t("dashboards.sales.remainingBalance")}</p>
                <p className="text-lg font-semibold text-ink">{formatCurrency(data.financial_summary.remainingBalance)}</p>
              </div>
              <div>
                ...
                <ul className="space-y-1.5">
                  {data.performance.byOwner.map((o) => (
                    <li key={o.ownerId ?? "unassigned"} className="flex items-center justify-between text-sm">
                      ...
                      <span className="text-ink-muted">{o.count} · {formatCurrency(o.value)}</span>
                    </li>
                  ))}
                </ul>
              )}
