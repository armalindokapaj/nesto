...
export default async function ReceivingPage() {
  ...
  const [movements, products, warehouses] = await Promise.all([
    db.inventoryMovement.findMany({ where: { tenantId, type: { in: ["RECEIPT", "ADJUSTMENT_INCREASE"] } }, orderBy: { date: "desc" }, include: { createdBy: { select: { displayName: true } } } }),
  ...
  return (
    <div className="space-y-6">
      {canWrite && (
        <Card>
          <CardHeader><div><CardTitle>{t("inventoryModule.newReceiving")}</CardTitle><CardDescription>{t("inventoryModule.receivingSubtitle")}</CardDescription></div></CardHeader>
          <CardContent>
            {products.length > 0 && warehouses.length > 0 ? (
              <MovementForm
                products={products.map((p) => ({ id: p.id, sku: p.sku, name: p.name }))}
                warehouses={warehouses.map((w) => ({ id: w.id, code: w.code, name: w.name }))}
                allowedTypes={["RECEIPT"]}
                redirectTo="/dashboard/inventory/receiving"
              />
            ) : (
      ...
      <Card>
        <CardHeader><CardTitle>{t("nav.receiving")}</CardTitle></CardHeader>
        <CardContent className="p-0">
          <Table>
            <THead><TRow><TH>{t("financeModule.journalNumber")}</TH><TH>{t("inventoryModule.reason")}</TH><TH>{t("common.status")}</TH><TH>{t("dashboards.admin.joined")}</TH></TRow></THead>
            <TBody>
              {movements.map((m) => (
                <TRow key={m.id}>
