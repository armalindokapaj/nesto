...
import { listSupplierQualifications } from "@/server/procurement";
...
export default async function SupplierQualificationPage() {
  ...
  const qualifications = await listSupplierQualifications(tenantId);
      ...
      <Card><CardContent className="p-0"><Table><THead><TRow><TH>Supplier</TH><TH>Category</TH><TH>Outcome</TH><TH>Valid Until</TH></TRow></THead><TBody>
        {qualifications.map((q) => (
            <TRow key={q.id}>
              ...
              <TD className={q.expiring ? "text-danger" : "text-ink-muted"}>{q.validUntil ? formatDate(q.validUntil) : "—"}</TD>
            </TRow>
        ))}
