...
export default async function ProcurementWorkspacePage() {
  ...
  const cards = [
    ...
    ...data.orders.filter((o) => !["CLOSED", "CANCELLED", "ARCHIVED"].includes(o.status)).map((o) => ({ id: o.id, stage: ["FULFILLED"].includes(o.status) ? "Completed" : ["PARTIALLY_FULFILLED"].includes(o.status) ? "Delivery" : "Ordered", kind: "Order", code: o.number, title: o.title ?? o.description, status: o.status, project: o.project?.name, value: o.amountMinor, currency: o.currency, date: o.requestedDeliveryDate, href: `/dashboard/procurement/orders/${o.id}` })),
