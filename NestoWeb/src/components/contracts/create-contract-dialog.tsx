...
type Props = {
  ...
  canCreateContractor?: boolean;
};

export function CreateContractDialog({ projects, contractors, canCreateContractor }: Props) {
  const { t } = useI18n();
  ...
  return (
    <Dialog.Root open={open} onOpenChange={setOpen}>
      <Dialog.Trigger asChild>
        <Button size="sm">
          <Plus size={14} /> {t("contracts.newContract")}
        </Button>
      </Dialog.Trigger>
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed left-1/2 top-1/2 z-50 w-[92vw] max-w-sm -translate-x-1/2 -translate-y-1/2 rounded-2xl border border-border bg-surface p-6 shadow-xl">
          <div className="flex items-center justify-between mb-4">
            <Dialog.Title className="text-base font-semibold text-ink">{t("contracts.newContract")}</Dialog.Title>
          ...
          >
            ...
            <div className="space-y-1.5">
              <Label htmlFor="value">{t("contracts.value")}</Label>
            ...
            <div className="space-y-1.5">
              <Label htmlFor="contractorId">{t("contractors.title")}</Label>
              <div className="flex items-center gap-2">
                ...
                {canCreateContractor && <CreateContractorDialog variant="inline" />}
              </div>
              {canCreateContractor && <p className="text-xs text-ink-faint">{t("contracts.contractorNotListed")}</p>}
            </div>
            {state?.error && (
