...
import { createProductCategoryAction } from "@/app/actions/inventory-module";
...
export function CreateCategoryDialog() {
  ...
  const [state, formAction, pending] = useActionState(createProductCategoryAction, undefined);

  return (
    <Dialog.Root open={open} onOpenChange={setOpen}>
      <Dialog.Trigger asChild>
        <Button size="sm" variant="secondary">
          <Plus size={14} /> {t("inventoryModule.newCategory")}
        </Button>
      </Dialog.Trigger>
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed left-1/2 top-1/2 z-50 w-[92vw] max-w-sm -translate-x-1/2 -translate-y-1/2 rounded-2xl border border-border bg-surface p-6 shadow-xl">
          <div className="mb-4 flex items-center justify-between">
            <Dialog.Title className="text-base font-semibold text-ink">{t("inventoryModule.newCategory")}</Dialog.Title>
          ...
          <form action={async (formData) => { await formAction(formData); setOpen(false); }} className="space-y-3">
            <div className="space-y-1.5">
              <Label htmlFor="cat-code">{t("inventoryModule.code")}</Label>
